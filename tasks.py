...
def grade_easy_task(db_path: str) -> Tuple[float, str]:
    ...
    try:
        ...
        if not cursor.fetchone():
            return 0.0, "Table 'users' does not exist"
        ...
        score = min(row_count / 10.0, 1.0)
        return score, f"Loaded {row_count}/10 rows into 'users' table"
    
    except Exception as e:
        return 0.0, f"Error grading easy task: {str(e)}"
...
def grade_medium_task(db_path: str) -> Tuple[float, str]:
    ...
    try:
        ...
        if not cursor.fetchone():
            return 0.0, "Table 'clean_sales' does not exist"
        ...
        if df.empty:
            return 0.0, "No data in 'clean_sales' table"
        
        # Check date format YYYY-MM-DD
        pattern = r'^\d{4}-\d{2}-\d{2}$'
        correct_dates = df['date'].str.match(pattern).sum()
        
        score = min(correct_dates / 20.0, 1.0)
        return score, f"Correct date format in {correct_dates}/20 rows"
    
    except Exception as e:
        return 0.0, f"Error grading medium task: {str(e)}"
...
def grade_hard_task(db_path: str) -> Tuple[float, str]:
    ...
    try:
        ...
        if not cursor.fetchone():
            return 0.0, "Table 'ltv_report' does not exist"
        ...
        if df.empty:
            return 0.0, "No data in 'ltv_report' table"
        ...
        score = min(correct_rows / 3.0, 1.0)
        return score, f"Correct LTV calculations for {correct_rows}/3 users"
    
    except Exception as e:
        return 0.0, f"Error grading hard task: {str(e)}"